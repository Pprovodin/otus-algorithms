...
public class CompareTrees {
    public static void main(String[] args) throws Exception {
        ...
        for (Integer size : sizes) {
            for (BinaryTree<Integer> tree : trees) {
                Runnable task = () -> testInsert(tree, size);
                stopwatch.start();
    ...
    private static void testInsert(BinaryTree<Integer> tree, int total) {
        new Random().ints(total, 1, 10_000).forEach(tree::insert);
//        IntStream.range(0, total).forEach(tree::insert);
    }
}
